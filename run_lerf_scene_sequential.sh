...
# Sequential ablation with an iteration budget matched to one joint stage:
#   1) RGB/ADMM-guided support sparsification (language loss disabled), then
#   2) semantic VQ optimization on the frozen, already-pruned support.
#
# Usage:
#   ./run_lerf_scene_sequential.sh SCENE [TOTAL_ITERS] [PORT_BASE] [SOURCE_CKPT]
#
# TOTAL_ITERS is split exactly between the two stages.  Override the first
# stage with SEQUENTIAL_SUPPORT_ITERATIONS; the semantic stage receives the
# remainder.  SOURCE_CKPT defaults to the scene's uncompressed three-scale
# checkpoint produced by run_lerf_scene_mvp.sh.
...
RUN_ROOT="${EXPERIMENT_ROOT}/clunogs_sequential/lerf_ovs/${SCENE_NAME}"
SUPPORT_DIR="${RUN_ROOT}/support_sparsification"
...
if [[ ! -f "${SUPPORT_CHECKPOINT}" ]]; then
    echo "missing support-stage checkpoint: ${SUPPORT_CHECKPOINT}" >&2
    exit 3
fi
...
# Stage two never changes the support, so its compact SH representation is
# exactly the one produced at the end of stage one.  Keep a checkpoint-matched
# name next to the final semantic checkpoint for the artifact exporter.
