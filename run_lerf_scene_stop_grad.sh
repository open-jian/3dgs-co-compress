...
# Joint baseline with the sole ablation that L_sem cannot update the shared
# Gaussian support.  Every other optimization/compression setting matches the
# full-joint stage in run_lerf_scene_mvp.sh.
#
# Usage:
#   ./run_lerf_scene_stop_grad.sh SCENE [ITERATIONS] [PORT] [SOURCE_CKPT]
...
RUN_ROOT="${EXPERIMENT_ROOT}/core_ablations/lerf_ovs/${SCENE_NAME}/joint_stop_semantic_support_grad"
...
printf '%s\n' \
    ...
    "pruning_fraction2=0.5" \
    "sh_codebook_size=256" \
    "stop_semantic_support_grad=true" \
