...
def eval_gt_ovsdata(mask_dir, output_path):
    mask_paths = []
    eval_index_list = []
    prompts = []
    
    for frame_id in os.listdir(mask_dir):
        mask_paths_one_frame = []
        if 'txt' in frame_id:
            continue
        mask_dir_one_frame = os.path.join(mask_dir, frame_id)
        eval_index_list.append(frame_id)
        
        for obj in os.listdir(mask_dir_one_frame):
            if obj == 'wood wall.png':
                continue
            mask_path = os.path.join(mask_dir_one_frame, obj)
            mask_paths_one_frame.append(mask_path)
            
        if 'wood wall.png' in os.listdir(mask_dir_one_frame):
            mask_paths_one_frame.append(os.path.join(mask_dir_one_frame, 'wood wall.png')) 
        
        mask_paths.append(mask_paths_one_frame)
        prompts.append(list(map(lambda x:os.path.split(x)[-1].split('.')[0], mask_paths_one_frame)))
    
    gt_ann = {}
    img_paths = []
    for i, idx in enumerate(eval_index_list):
        img_ann = defaultdict(dict)
        prompts_one_frame = prompts[i]
        mask_paths_one_frame = mask_paths[i]
        for j, prompt in enumerate(prompts_one_frame):
            mask_gt = cv2.imread(mask_paths_one_frame[j])
            mask_gt[mask_gt == 255] = 1
            mask_gt = mask_gt[:, :, 0]
            img_ann[prompt]['mask'] = mask_gt

            save_path = output_path / 'gt' / idx / f'{prompt}.jpg'
            save_path.parent.mkdir(exist_ok=True, parents=True)
            vis_mask_save(mask_gt, save_path)
...
def segmentation_process_cuda_room(sem_map:torch.tensor, clip_model, thresh, img_ann, prompts, image_name):
    print("processing scene room")
    ...
    for k in range(n_prompt - 2):
        ...
        for i in range(n_head):
            ...
            mask_gt = img_ann[prompts[k]]['mask']
            mask_gt = cv2.resize(mask_gt, (w, h), interpolation=cv2.INTER_NEAREST)
            mask_gt = torch.from_numpy(mask_gt.astype(np.uint8)).to(device)
    ...
    for i in range(n_head):
        ...
        valid_map[i][k] = 0.5 * (avg_filtered.squeeze(0).squeeze(0) + valid_map[i][k])
        # truncate the heatmap into mask
        output_a = valid_map[i][k]
        output_a = output_a - torch.min(output_a)
        output_a = output_a / (torch.max(output_a) + 1e-9)
        output_a = output_a * (1.0 - (-1.0)) + (-1.0)
        output_a = torch.clip(output_a, 0, 1)

        output_b = valid_map[i][k+1]
        output_b = output_b - torch.min(output_b)
        output_b = output_b / (torch.max(output_b) + 1e-9)
        output_b = output_b * (1.0 - (-1.0)) + (-1.0)
        output_b = torch.clip(output_b, 0, 1)

        mask_pred_a = (output_a > thresh).type(torch.uint8)
        mask_pred_b = (output_b > thresh).type(torch.uint8)
        mask_pred = torch.logical_or(mask_pred_a, mask_pred_b)
        mask_pred = smooth_cuda(mask_pred)
        mask_lvl[i] = mask_pred
...
def evaluate(dataset:ModelParams, pipeline:PipelineParams, args):
    ...
    # load test data
    gt_ann, image_paths = eval_gt_ovsdata(args.gt_mask_dir, Path(args.output_path))
    print(gt_ann.keys())
    ...
    for i, idx in enumerate(tqdm(eval_index_list)):
        image_name = Path(args.output_path) / f'{idx:0>2}'
        image_name.mkdir(exist_ok=True, parents=True)
        img_ann = gt_ann[f'{idx:0>2}']
        if args.dataset_name == 'room':
            clip_model.set_positives(list(img_ann.keys()) + ['wood table'])
        else:
        ...
        if args.dataset_name == 'room':
            c_iou_list, c_lvl = segmentation_process_cuda_room(restored_feat, clip_model, args.mask_thresh, img_ann, list(img_ann.keys()), image_name)
        else:
    ...
    prompt_length = len(list(img_ann.keys()))
    if args.dataset_name == 'room':
        prompt_length -= 1
    prompt_mean_iou = []
    for idx in range(prompt_length):
        total = 0
        total_num = 0
        for i, iou in enumerate(chosen_iou_all):
            if i % prompt_length == idx:
                total += iou
                total_num += 1
        prompt_mean_iou.append(total / total_num)
    logger.info(f"prompt_mean_iou_list: \n{prompt_mean_iou}")
    logger.info(f"prompt_list: \n{list(img_ann.keys())}")
